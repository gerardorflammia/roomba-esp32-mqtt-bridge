...
#define BRC_PIN 14     // Pin para "Baud Rate Change" / "Device Detect" (Pin 5 del Mini-DIN)
...
unsigned long lastReconnectAttempt = 0; // Almacena el último intento de conexión MQTT
const unsigned long reconnectInterval = 5000; // Intervalo de 5 segundos entre reconexiones

// --- Variables de Seguridad (Watchdog) ---
unsigned long lastMsgTime = 0; // Almacena la hora del último mensaje MQTT recibido
const unsigned long FAILSAFE_TIMEOUT = 30000; // Tiempo límite de 30 segundos de inactividad
...
void callback(char *topic, byte *payload, unsigned int length); // Pre-declaración: Procesamiento MQTT
...
void setup() {                                    // Función de configuración inicial (corre una vez)
  ...
  Serial.println("\n--- Iniciando Roomba ESP32 Bridge ---"); // Mensaje de bienvenida en consola
  ...
  // Parámetros: Baudios (115200), Modo (8 bits, sin paridad, 1 bit parada), Pines RX/TX
  Serial2.begin(115200, SERIAL_8N1, RX2_PIN, TX2_PIN); // Inicia comunicación con Roomba
  Serial.println("Serial2 iniciado para Roomba (RX:16, TX:17)"); // Confirma pines en log
  ...
  client.setServer(mqtt_server, mqtt_port);      // Define dirección y puerto del broker
...
void loop() {
  ...
  if (!client.connected()) {           // Si el cliente MQTT se ha desconectado
    ...
    if (now - lastReconnectAttempt > reconnectInterval) { // Si han pasado 5 segundos
    ...
    /* DESACTIVADO TEMPORALMENTE PARA PRUEBAS (Evitaba el movimiento libre en debug)
    if (millis() - lastMsgTime > FAILSAFE_TIMEOUT && !failsafeTriggered) { // Si hay silencio largo
      Serial.println("FAILSAFE: Timeout. Stopping."); // Informa el error por USB
      Serial2.write(173); // OpCode 173: Detiene el sistema OI completamente por seguridad
      failsafeTriggered = true; // Marca que el seguro se ha activado
    }
    */
    ...
    if (millis() - last_sensor_time > 2000) {
      ...
      // Packet ID 3: Solicita un grupo de 10 sensores (incluye batería, voltaje, carga, etc.)
      Serial2.write(142); 
      Serial2.write(3);   
      ...
      if (Serial2.available() >= 10) { // Si hay 10 bytes (tamaño del paquete 3)
        byte buf[10];
        Serial2.readBytes(buf, 10); // Lee los 10 bytes y los guarda en el buffer

        // Decodificación de los bytes según el manual "OI Spec"
        uint8_t chargingState = buf[0];                // Estado de carga (0: No cargando, 1: Recuperación, 2: Carga, 3: Goteo, 4: Espera, 5: Error)
        uint16_t voltage = (buf[1] << 8) | buf[2];     // Combina Byte Alto y Bajo para obtener milivoltios de batería
        int16_t current = (buf[3] << 8) | buf[4];      // Combina bytes para obtener corriente (Positivo: Carga, Negativo: Consumo)
        uint16_t charge = (buf[6] << 8) | buf[7];      // Obtiene la carga actual almacenada en mAh
        uint16_t capacity = (buf[8] << 8) | buf[9];     // Capacidad máxima de diseño de la batería en mAh

        int battery_percent = 0;                       // Inicializa variable de porcentaje
        if (capacity > 0)                              // Evita división por cero
          battery_percent = (charge * 100) / capacity; // Calcula el porcentaje relativo al máximo
        if (battery_percent > 100)                     // Corrección si supera el 100%
          battery_percent = 100;

        String statusStr = "idle";                     // Inicializa cadena de estado en reposo

        if (chargingState > 0 && chargingState < 4) {  // Si el robot está en proceso de carga activa
        ...
        } else if (chargingState == 4) {               // Si el robot está conectado pero cargado
        ...
        snprintf(jsonBuffer, sizeof(jsonBuffer),       // Formatea el JSON con las variables leídas
...
// wakeUpRoomba: Secuencia de pulsos eléctricos y comandos OI para encender el robot
void wakeUpRoomba() {
  Serial.println("Despertando Roomba...");         // Imprime en el log de depuración
  // El robot entra en reposo si no se usa. Para despertarlo, el pin BRC (Device Detect)
  // debe recibir un pulso LOW de al menos 500ms.
...
void setup_wifi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {   // Si el estado de WiFi es conectado exitosamente
    Serial.println("\nConectado. IP: "); // Imprime mensaje de confirmación
    Serial.println(WiFi.localIP());      // Muestra la dirección IP local del dispositivo
...
boolean reconnect() {                             // Función para reconexión persistente a MQTT
  String clientId = "ESP32Roomba-" + String(random(0xffff), HEX); // Crea un ID aleatorio único
  if (client.connect(clientId.c_str(), mqtt_user, mqtt_password)) { // Ejecuta intento de conexión con credenciales
    Serial.println("MQTT Conectado");   // Registra éxito en el broker por puerto serial
    client.subscribe(topic_commands);    // Se asocia al tema de escucha para recibir órdenes
...
void callback(char *topic, byte *payload, unsigned int length) {
  ...
  if (String(topic) == topic_commands) {                // Compara si el tópico es el de comandos
    // --- Comandos START / TURN_ON ---
    if (message == "start" || message == "turn_on") {           // Opción: Iniciar limpieza
    ...
    // --- Comandos STOP / TURN_OFF ---
    else if (message == "stop" || message == "turn_off") {       // Opción: Detener robot
      ...
      Serial.println("Action: Stop (Robust: Safe Mode + Drive 0)"); // Log por USB
    }                                                           // Fin comando stop
    ...
    } else if (message == "dock" || message == "return_to_base") { // Comando volver a base
      wakeUpRoomba();                                   // Asegura que el robot esté despierto
      delay(500);                                       // Pausa de seguridad
    ...
    // --- Comando POWER_DOWN ---
      ...
      Serial2.write(133); // OpCode 133: POWER DOWN (Pone al robot en hibernación profunda)
      Serial.println("Action: Power Down");               // Informa la desconexión por consola
...
void move(int velocity, int radius) {
  ...
  Serial2.write((radius >> 8) & 0xFF);   // Envía los 8 bits más significativos del radio
  Serial2.write(radius & 0xFF);          // Envía los 8 bits menos significativos del radio
...
void playSong(int songNumber) {          // Función para ejecutar sonidos almacenados
  ...
  Serial2.write(songNumber);             // Valor de la canción (0 a 15 permitidos en iRobot)
